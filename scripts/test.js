...
class ExtensionTester {
  ...
  async testFiles() {
    ...
    const requiredFiles = ['background.js', 'content.js', 'popup.html', 'popup.css', 'popup.js'];
    ...
    const optionalFiles = ['README.md', 'package.json', 'LICENSE'];
    ...
    for (const file of jsFiles) {
      ...
      this.test(`${file} has proper structure`, () => {
        ...
        if (file === 'background.js') {
          ...
          return content.includes('document.querySelector') || content.includes('window.location');
        } else if (file === 'popup.js') {
          return (
            content.includes('document.getElementById') || content.includes('addEventListener')
          );
        }
  ...
  async testHTML() {
    ...
    this.test('popup.html has required elements', () => {
      const content = fs.readFileSync(htmlPath, 'utf8');
      return (
        content.includes('start-scan-btn') &&
    ...
    this.test('popup.html has proper meta tags', () => {
      const content = fs.readFileSync(htmlPath, 'utf8');
      return content.includes('charset="UTF-8"') && content.includes('viewport');
    });
...
class FileSizeAnalyzer {
  ...
  analyze() {
    ...
    if (fs.existsSync(iconsDir)) {
      ...
      if (iconSize > 0) {
        console.log(
          `icons/ (${iconFiles.length} files)`.padEnd(20) + `: ${this.formatBytes(iconSize)}`
        );
        totalSize += iconSize;
      }
    }
    ...
    for (const file of jsFiles) {
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (stats.size > 50 * 1024) {
          suggestions.push(
            `${file} is large (${this.formatBytes(stats.size)}). Consider code splitting.`
          );
        }
